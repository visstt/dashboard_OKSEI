...
import GroupsFilters from "@/widgets/groupsFilters/GroupsFilters";
...
function AllGroupsPage() {
  ...
  const filteredGroupData = useMemo(() => {
    ...
    return stats.groupData.filter(({ group }) => {
      ...
      if (
        selectedDepartment &&
        (stats.groupToDepartment[group].length > 30
          ? stats.groupToDepartment[group].substring(0, 27) + "..."
  ...
  const groupCards = useMemo(() => {
    return paginatedGroups.map(({ group }) => (
      <CardWrapper key={group} onClick={() => goToGroup(group)}>
  ...
  const departmentsList = useMemo(
    () => (stats ? stats.deptData.map(({ department }) => department) : []),
    [stats]
  );
  ...
  const handleDepartmentSelect = useCallback((dep: string) => {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <GroupsFilters
            ...
            groupSearchValue={searchValue}
            onChangeGroupSearch={setSearchValue}
        ...
        <div
          style={{
            gridTemplateColumns:
              "repeat(auto-fill,minmax(clamp(560px, calc((100vw - 255px) / 4), 350px), 1fr))",
          }}
        ...
        {limit < filteredGroupData.length && (
          <div className="flex justify-center">
            <button
              ...
              className={cn(
                "mt-4 rounded-md border px-6 py-2",
